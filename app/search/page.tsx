...
// Auth gate is enforced by Clerk middleware (middleware.ts) — unauthenticated
// users on configured deploys are redirected to /sign-in before this page runs.
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-6xl px-5 pt-8 pb-4">
        ...
        <SearchInput initial={q} autoFocus={false} size="md" />
      </section>
    ...
    return (
      <section className="mx-auto max-w-6xl px-5 py-12 text-center">
        <p className="text-ink-mute">
          Try a search like “React beginner frontend issues”.
        </p>
      </section>
    );
  }
...
async function runSearch(q: string): Promise<SearchResponse> {
  // 60s identical-query cache. Cuts GitHub + OpenRouter spend on duplicates.
